...
class Command extends ConsoleCommand
{
    ...
    {
        if ($this->option('host')) {
            $this->hostModel = $this->option('host') == time() . '-test' ? ServerHost::first() : ServerHost::where('name', $this->option('host'))->first();
            if ($this->hostModel) {
    ...
    {
        if ($this->option('user')) {
            $this->userModel = $this->option('user') == time() . '-test' ? ServerUser::first() : ServerUser::where('name', $this->option('user'))->first();
            if ($this->userModel) {
    ...
    {
        if ($this->option('app')) {
            $this->appModel = $this->option('app') == time() . '-test' ? ServerApp::first() : ServerApp::where('name', $this->option('app'))->first();
            if ($this->appModel) {
    ...
    public function askDatabase()
    {
        if ($this->option('database')) {
            $this->databaseModel = $this->option('database') == time() . '-test' ? ServerDatabase::first() : ServerDatabase::where('name', $this->option('database'))->first();
